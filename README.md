...
## Using with Google Gemini (Special Instructions)

When using Gemini as your LLM provider, follow these specific steps to avoid common issues:

### 1. Set up your Gemini API key

Make sure you have your Gemini API key properly set up in the environment:
...
### 2. Always specify the provider and model

When using the CLI, always explicitly specify Gemini as the provider and use an appropriate Gemini model:

```bash
droidrun "Open the calculator app" --provider gemini --model gemini-2.0-flash
```

The `--provider gemini` flag is crucial - without it, DroidRun may default to OpenAI.

### 3. Verify your environment

To check if your Gemini API key is properly loaded in the environment:

```bash
echo $GEMINI_API_KEY
```

This should display your API key. If not, the environment variable was not set correctly.

### 4. Using Gemini in a script

When using DroidRun with Gemini in a Python script, use this template:
...
async def main():
    # Create the LLM reasoner with explicit provider, model and API key
    llm = LLMReasoner(
        llm_provider="gemini",  # Must specify "gemini" here
