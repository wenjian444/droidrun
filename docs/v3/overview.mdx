...
We have published our latest benchmark results at [droidrun.ai/benchmark](https://droidrun.ai/benchmark). Explore detailed performance metrics and review execution trajectories for every task. See how DroidRun delivers real, measurable results — no hype, just proven capabilities.
...
<CardGroup cols={2}>
  ...
  <Card title="Multiple LLM Support" icon="brain">
    Support for OpenAI, Anthropic, GoogleGenAI, Ollama, and Deepseek
  </Card>
...
droidrun "Open calculator app" --provider GoogleGenAI --model models/gemini-2.5-pro
...
async def main():
    ...
    llm = load_llm(
        provider_name="GoogleGenAI",  # OpenAI, ollama, Anthropic, GoogleGenAI, DeepSeek
