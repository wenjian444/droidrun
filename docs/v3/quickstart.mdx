...
```bash
# Using default (GoogleGenAI)
droidrun "Open the settings app"

# Using specific provider and model
droidrun "Open calculator" --provider GoogleGenAI --model models/gemini-2.5-pro

# With reflection and planning
droidrun "Take a screenshot and describe what's on the screen" --reflection --reasoning
...
# Enable reflection
droidrun "Analyze screen" --reflection
...
async def main():
    ...
    llm = load_llm(
        provider_name="GoogleGenAI",  # Case sensitive: OpenAI, Ollama, Anthropic, GoogleGenAI, DeepSeek
